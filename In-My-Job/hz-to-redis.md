...
#### 1. 성숙도
![[Pasted image 20221008130804.png]]
...
#### 3. 개발 편의성
Redis하면 가장 먼저 특징으로 거론되는게 다양한 자료구조와 Singleton 패턴이다. 개발에 있어서 이런 부분은 많은 수고를 덜어준다. 특히 원자성 보장은 기존 race condition 관리를 위한 수고를 눈에 띄게 덜어줬다.
...
*결국 은탄은 없다는 것이다...*
![[Pasted image 20221008134535.png]]
![[Pasted image 20221008140033.png]]
...
이는 Hazelcast를 사용하던 시절의 코드를 거의 그대로 매핑한 것인데, Redisson에는  `RAtomicLong`과 같은 변수 증분용 캐시 오브젝트가 있다. 게다가 이는 Redis의 Singleton 특성에 따라 원자성이 보장된다.
