...
1. 인터페이스에 정의된 바인딩 네임(==RabbitMQ exchange) 탐색
2. 해당하는 바인딩을 yml, properties 등에서 탐색
3. 탐색된 바인딩의 topic으로 메세지 전달이 가능해짐
...
1. **Injecting Bound Interface**
    ...
    예제에서 requestApi 빈은 request 호출 시 output 채널에 메세지를 전송한다. 이는 주입된 `RequestApi` Bean에서 타겟 채널을 찾기 위해 request를 호출한다.
